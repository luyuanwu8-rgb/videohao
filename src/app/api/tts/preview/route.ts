...
export async function POST(req: NextRequest) {
  await ensureConfigLoaded(); // 确保设置页填入库的密钥可被 env() 读到
  const body = await req.json().catch(() => ({}));
  const provider =
    body.provider === "stepfun" ? "stepfun" : body.provider === "aurastd" ? "aurastd" : "volcengine";
  ...
  try {
    ...
    } else if (provider === "aurastd") {
      await synthesizeAura(text, dest, "real", {
        voice,
        speed,
        vol: typeof body.vol === "number" ? body.vol : undefined,
        pitch: typeof body.pitch === "number" ? body.pitch : undefined,
        emotion: typeof body.emotion === "string" ? body.emotion : undefined,
        voiceModify: body.voiceModify && typeof body.voiceModify === "object" ? body.voiceModify : undefined,
      });
    } else {
