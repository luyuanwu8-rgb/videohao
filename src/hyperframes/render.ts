...
export async function renderTimeline(input: RenderInput): Promise<RenderResult> {
  ...
  // 渲染前清理 renders 下的残留 work-* 临时目录和旧 mp4，避免 latestMp4 取错 + 磁盘堆积
  await cleanRendersDir(rendersDir);
  ...
  const code = await runHyperframes(
    [
      "--yes",
  ...
  if (code !== 0) {
    return { ok: false, error: `hyperframes render 退出码 ${code}` };
  }
...
/** 清理 renders 目录下的残留 work-* 临时目录（渲染前调用，防句柄堆积/磁盘膨胀）。
 * 注意：不删 .mp4 成品——多动效批量时各条成品都留在 renders/ 供画廊展示。 */
async function cleanRendersDir(dir: string): Promise<void> {
  if (!existsSync(dir)) return;
  const entries = await readdir(dir).catch(() => [] as string[]);
  for (const name of entries) {
    if (name.startsWith("work-")) {
      await rm(resolve(dir, name), { recursive: true, force: true }).catch(() => {});
    }
  }
}
